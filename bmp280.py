...
# Use cases (see paragraph 3.4 in datasheet)
...
class BMP280:
    ...
    def _gauge(self):
        ...
        # Oversampling = 0. Happens after reset
        if d[:3] == bytes((0x80, 0x00, 0x00)):
            self.initialize()
            d = self._read(_BMP280_REGISTER_DATA, 6)
    ...
    def pressure(self):
        ...
        if self._p == 0:
            ...
            self._p = p / 256.0
        return self._p / 100 # давление в мбар
